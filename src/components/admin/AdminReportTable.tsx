...
const AdminReportTable = ({ posts, handleBlock, handleUnblock, }: Props) => {
      ...
      <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
          <tr>
            ...
            <th scope="col" className="Capitalize px-6 py-3">block</th>
          </tr>
        </thead>

        <tbody>
          {posts.map((post) => (
            <tr key={post._id} className={`bg-white border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 ${post.reportsCount > 10 ? 'dark:bg-red-900' :'dark:bg-gray-800'}`}>
